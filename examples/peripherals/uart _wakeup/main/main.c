/**
 * @brief       main.c
 * @details     This example shows how to work with the UART wakeup functionality.
 *
 ...
 * 				
 * 				The MCU remains in light-sleep mode otherwise.		
 * 
 *
 * @return      N/A
 *
 ...
 * @pre         UART wakeup in Light-sleep mode needs to send an extra character to wake the mcu up like '-'.
 * @warning     N/A.
...
void app_main(void)
{
	...
	while (1) {    
		...
		if (uart_data_len > 0U)
		{
			...
			/* Read RX buffer	*/
			uart_data_len = uart_read_bytes(UART_NUM_1, data, uart_data_len, 0U);
